...
/**
 * Extrae posts de LinkedIn usando Apify
...
const extractPostsFromProfiles = async (profileUrls) => {
  ...
  try {
    ...
    // Procesar resultados por perfil
    ...
    for (const item of items) {
      ...
      if (!resultsByProfile[normalizedProfileUrl]) {
        resultsByProfile[normalizedProfileUrl] = {
      ...
      if (item.url || item.postUrl || item.linkedinUrl) {
        resultsByProfile[normalizedProfileUrl].posts.push({
          url: item.url || item.postUrl || item.linkedinUrl,
          text: item.text || item.content || item.description || '',
          author: authorName || resultsByProfile[normalizedProfileUrl].profileName,
    ...
    // Convertir a array
    const results = Object.values(resultsByProfile);

    loggerService.info(`\n=== RESUMEN DE EXTRACCIÓN ===`);
    loggerService.info(`Perfiles procesados: ${results.length}`);
    loggerService.info(`Total posts extraídos: ${items.length}`);
